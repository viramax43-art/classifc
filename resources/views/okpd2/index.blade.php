...
<body x-data="okpd2App()" x-init="init()">
    <div class="app">
        <header class="topbar">
            <h1>ОКПД 2</h1>
            <nav style="display:flex;gap:8px;margin-left:auto;font-size:0.85rem">
                <span style="color:var(--primary);font-weight:500">ОКПД 2</span>
                <span style="color:var(--border)">|</span>
                <a href="{{ $tnvedUrl }}" style="color:var(--muted);text-decoration:none">ТН ВЭД</a>
            </nav>
        </header>
        ...
        @if($totalCount === 0)
            <div class="alert">База пуста — <code>php artisan okpd2</code></div>
        @endif
        ...
        <div class="layout">
                ...
                <div class="sidebar-scroll">
                    <div x-show="query.length > 0" x-cloak>
                        ...
                        <template x-for="(item, index) in searchResults" :key="item.code">
                            ...
                            >
                                <span class="code wide" x-html="highlight(item.code)"></span>
                    ...
                    <div x-show="query.length === 0">
                        ...
                        <template x-for="section in sections" :key="section.code">
                            <button
                                type="button"
                                class="row-btn"
                                :class="{ active: activeSection === section.code && !selected }"
                                @click="openSection(section.code)"
                            >
                                <span class="code" x-text="section.code"></span>
                                <span class="row-text" x-text="section.name"></span>
                                <span class="badge" x-text="section.count"></span>
                            </button>
                        </template>
            ...
            <main class="panel main">
                <div class="welcome" x-show="!selected && !activeSection">
                    <div class="welcome-icon">📋</div>
                    <div class="welcome-title">Классификатор ОКПД 2</div>
                    <div class="welcome-sub">ОК 034-2014 (КПЕС 2008)</div>
                    <div class="welcome-hint">Выберите раздел слева или воспользуйтесь поиском</div>
                    <div class="welcome-kbd">Нажмите <kbd>/</kbd> для быстрого поиска</div>
                ...
                <div x-show="selected" x-cloak>
                    ...
                    <ul class="head-breadcrumb" x-show="navigableBreadcrumb().length">
                        <li>
                            <button type="button" @click="goHome()" title="К списку разделов">ОКПД 2</button>
                        </li>
                        <template x-for="crumb in navigableBreadcrumb()" :key="crumb.code + (crumb.is_section ? '_s' : '')">
                            <li>
                                <template x-if="crumb.code !== selected.code">
                                    <button type="button" :title="crumb.name" @click="openCrumb(crumb)" x-text="crumb.code"></button>
                                </template>
                                <template x-if="crumb.code === selected.code">
                                    <span class="crumb-current" :title="crumb.name" x-text="crumb.code"></span>
                                </template>
                            </li>
                        </template>
                    </ul>
                    ...
                    <div class="detail">
                        ...
                        <table class="head-table">
                            ...
                            <tbody>
                                <tr>
                                    <td data-label="Классификатор">ОКПД 2</td>
                                    <td data-label="Код раздела" x-text="selected.section"></td>
                                    <td data-label="Код" x-text="selected.code"></td>
                        ...
                        <div class="toolbar">
                            <div class="toolbar-group">
                                ...
                                <button type="button" class="btn btn-primary" @click="copyText(selected.code)" title="Скопировать код">Код</button>
                                <button type="button" class="btn" @click="copyText(`${selected.code} — ${selected.name}`)" title="Скопировать код и название">Код + название</button>
                        ...
                        <div x-show="relatedTnved.length > 0">
                            <div class="block-title">Связанные коды ТН ВЭД</div>
                            @include('partials.actualization', ['date' => $mappingsUpdatedAt, 'compact' => true])
                            <table class="children-table">
                                <tbody>
                                    <template x-for="item in relatedTnved" :key="item.code">
                                        <tr>
                            ...
                            <table class="children-table">
                                <tbody>
                                    <template x-for="child in children" :key="child.code">
                                        <tr>
                                            <td class="td-code" x-text="child.code"></td>
                                            <td>
                        ...
                        <div class="scheme-tree">
                            ...
                            <div class="scheme">
                                <p>Схема иерархии в классификаторе ОКПД 2 для кода <span x-text="selected.code"></span>:</p>

                                <div class="scheme-tree-row scheme-tree-level-0 scheme-folder" @click="goHome()">
                                    <span class="scheme-icon">📂</span>
                                    <span>ОКПД 2</span>
                                    <span class="scheme-tree-muted">ОК 034-2014</span>
                                </div>
                                ...
                                <template x-for="(node, idx) in schemeNodes()" :key="`scheme_${node.code}_${idx}`">
                                    <div>
                                        <div class="scheme-tree-row scheme-folder"
                                             ...
                                             @click="node.code !== selected.code && (node.is_section ? openSection(node.code) : openCode(node.code))">
                                            <span>↳</span>
                                            <span class="scheme-icon" x-text="schemeNodeIcon(node)"></span>
                                            <span class="scheme-tree-text">
                                                <strong x-text="node.code"></strong>
                ...
                <div class="detail" x-show="!selected && activeSection && treeItems.length > 0" x-cloak>
                    @include('partials.actualization', ['date' => $classifierUpdatedAt, 'compact' => true])
                    <h1 class="head-title" x-text="`Раздел ${activeSection} — ${sectionTitle(activeSection)}`"></h1>
                    <p style="font-size:0.82rem;color:var(--muted);margin-bottom:10px">Классов: <strong x-text="treeItems.length"></strong></p>

                    <div class="list">
                        <template x-for="row in flattenItems(treeItems)" :key="`${row.code}_${row.depth}`">
                            <div class="tree-row">
                                <button
                                    type="button"
                                    class="tree-toggle"
                                    :class="{ 'is-placeholder': !row.has_children }"
                                    :disabled="!row.has_children"
                                    @click="row.has_children && toggleExpand(row.code)"
                                    x-text="row.has_children ? (isExpanded(row.code) ? '▼' : '▶') : ''"
                                ></button>
                                <button type="button" class="row-btn" :style="`padding-left:${8 + row.depth * 12}px`" @click="onTreeRowClick(row)" @dblclick.prevent="openCode(row.code)" :title="row.has_children ? 'Щелчок — развернуть уровень, двойной щелчок — открыть карточку' : ''">
                                    <span class="code wide" @click.stop="openCode(row.code)" x-text="row.code"></span>
                                    <span class="row-text" x-text="row.name"></span>
                                </button>
                            </div>
                        </template>
                    </div>
            ...
            return {
                ...
                async search() {
                    ...
                    try {
                        ...
                        const data = await (await fetch(`/api/okpd2/search?${params}`)).json();
                ...
                async openSection(code) {
                    const data = await (await fetch(`/api/okpd2/children?section=${code}`)).json();
                    const items = data.items || [];

                    if (items.length === 0) {
                        this.showToast('В разделе нет позиций');
                        return;
                    }
                ...
                async openCode(code) {
                    const response = await fetch(`/api/okpd2/${encodeURIComponent(code)}`);
                    if (!response.ok) {
                    ...
                    this.relatedTnved = data.related_tnved || [];
                ...
                async openCrumb(crumb) {
                    if (crumb.is_section) {
                        await this.openSection(crumb.code);
                        return;
                    }
                ...
                isExpanded(code) {
                    return this.expandedCodes.includes(code);
                },

                onTreeRowClick(row) {
                    if (row.has_children) {
                        this.toggleExpand(row.code);
                        return;
                    }
                ...
                async toggleExpand(code) {
                    if (this.isExpanded(code)) {
                        this.expandedCodes = this.expandedCodes.filter(c => c !== code);
                        return;
                    }

                    if (!this.childCache[code]) {
                        const data = await (await fetch(`/api/okpd2/children?parent=${encodeURIComponent(code)}`)).json();
                        this.childCache[code] = data.items || [];
                    }

                    this.expandedCodes = [...this.expandedCodes, code];
                },
                ...
                flattenItems(items, depth = 0) {
                    ...
                    for (const item of items || []) {
                        ...
                        if (this.isExpanded(item.code) && this.childCache[item.code]?.length) {
                            rows.push(...this.flattenItems(this.childCache[item.code], depth + 1));
                        }
                    }
                ...
                loadHistory() {
                    try { return JSON.parse(localStorage.getItem('okpd2_history') || '[]'); }
                    catch { return []; }
                },

                pushHistory(item) {
                    this.history = [{ code: item.code, name: item.name }, ...this.history.filter(h => h.code !== item.code)].slice(0, 6);
                    localStorage.setItem('okpd2_history', JSON.stringify(this.history));
                },
                ...
                schemeNodes() {
                    ...
                    return this.selected.breadcrumb
                        .map(node => ({ code: node.code, name: node.name || node.code, is_section: !!node.is_section }));
                },
                ...
                childrenIntroText() {
                    ...
                    return `Запись в классификаторе с кодом ${this.selected.code} содержит ${count} уточняющих (дочерних) ${word}.`;
                },
